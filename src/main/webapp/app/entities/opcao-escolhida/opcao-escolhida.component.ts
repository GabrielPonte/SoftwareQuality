import { mixins } from 'vue-class-component';

import { Component, Vue, Inject } from 'vue-property-decorator';
import Vue2Filters from 'vue2-filters';
import { IOpcaoEscolhida } from '@/shared/model/opcao-escolhida.model';

import OpcaoEscolhidaService from './opcao-escolhida.service';

@Component({
  mixins: [Vue2Filters.mixin],
})
export default class OpcaoEscolhida extends Vue {
  @Inject('opcaoEscolhidaService') private opcaoEscolhidaService: () => OpcaoEscolhidaService;
  private removeId: number = null;

  public opcaoEscolhidas: IOpcaoEscolhida[] = [];

  public isFetching = false;

  public mounted(): void {
    this.retrieveAllOpcaoEscolhidas();
  }

  public clear(): void {
    this.retrieveAllOpcaoEscolhidas();
  }

  public retrieveAllOpcaoEscolhidas(): void {
    this.isFetching = true;

    this.opcaoEscolhidaService()
      .retrieve()
      .then(
        res => {
          this.opcaoEscolhidas = res.data;
          this.isFetching = false;
        },
        err => {
          this.isFetching = false;
        }
      );
  }

  public handleSyncList(): void {
    this.clear();
  }

  public prepareRemove(instance: IOpcaoEscolhida): void {
    this.removeId = instance.id;
    if (<any>this.$refs.removeEntity) {
      (<any>this.$refs.removeEntity).show();
    }
  }

  public removeOpcaoEscolhida(): void {
    this.opcaoEscolhidaService()
      .delete(this.removeId)
      .then(() => {
        const message = this.$t('softwareQualityApp.opcaoEscolhida.deleted', { param: this.removeId });
        this.$bvToast.toast(message.toString(), {
          toaster: 'b-toaster-top-center',
          title: 'Info',
          variant: 'danger',
          solid: true,
          autoHideDelay: 5000,
        });
        this.removeId = null;
        this.retrieveAllOpcaoEscolhidas();
        this.closeDialog();
      });
  }

  public closeDialog(): void {
    (<any>this.$refs.removeEntity).hide();
  }
}
